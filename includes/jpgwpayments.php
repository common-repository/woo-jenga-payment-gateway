    ...
    function __construct(){
        ...
        add_action('admin_head', 'my_column_width');
            function my_column_width() {
                ...
                echo '.wp-list-table .column-order_reference { width: 30%; }';
                echo '.wp-list-table .column-transaction_date { width: 40%; }';
                echo '.wp-list-table .column-transaction_reference { width: 40%; }';
                echo '.wp-list-table .column-payment_channel { width: 40%; }';
                echo '.wp-list-table .column-transaction_amount { width: 30%; }';
                echo '.wp-list-table .column-transaction_currency { width: 30%; }';
   ...
   // Define the  Search Box that will appear on the Transactions table
    public function search_box( $text, $input_id ) {
        ...
        <p class="search-box">
            ...
            <input type="search" id="<?php echo esc_attr( $input_id ); ?>" name="s" placeholder="order reference" value="<?php _admin_search_query(); ?>" />
    ...
    public function get_columns(){
        $columns = array(
            'order_reference' => __( 'Order Ref', 'mylisttable' ),
            'order_status'    => __( 'Status', 'mylisttable' ),
            'transaction_reference' => __( 'Transaction Ref', 'mylisttable' ),
            'transaction_amount'      => __( 'Amount', 'mylisttable' ),
            'transaction_currency'      => __( 'Currency', 'mylisttable' ),
            'payment_channel'      => __( 'Channel', 'mylisttable' ),
            'transaction_date'      => __( 'Transaction Date', 'mylisttable' )
        );
        return $columns;
    }
    ...
    public function get_sortable_columns() {
        $sortable_columns = array(
            'order_reference'  => array('order_reference',false),
            'order_status' => array('order_status',false),
            'transaction_reference'   => array('transaction_reference',false),
            'transaction_amount'   => array('transaction_amount',false),
            'transaction_currency'   => array('transaction_currency',false),
            'payment_channel'   => array('payment_channel',false),
    ...
    public function prepare_items() {
            ...
            $page_number = 1;
            $appTable = $wpdb->prefix . "jpgw_trx";
            $do_search = ( $search ) ? $wpdb->prepare(" SELECT * FROM $appTable WHERE order_reference LIKE '%%%s%%' ", $search ) : '';


//            if ( ! empty( $_REQUEST['orderby'] ) ) {
//                $do_search .= ' ORDER BY ' . esc_sql( $_REQUEST['orderby'] );
//                $do_search .= ! empty( $_REQUEST['order'] ) ? ' ' . esc_sql( $_REQUEST['order'] ) : ' DESC';
//            }
//
